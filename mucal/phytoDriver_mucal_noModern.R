...
# Incorporates only GIG data to calibrate mui = f(radius, po4)
# Dustin T. Harper
############################################################################################  
...
############################################################################################
inv.out = jags.parallel(data = data.pass, model.file = "phytoPSM_mucal_noModern.R", parameters.to.save = parms,
...
write.csv(coeff.mat, file = "model_out/coeff_mui_noModern.csv")
